...
#if 0 /* @SURE - RSW Code => */
#define PSRAM_RSW_CODE_ADDR		0x70101000 // psram0
#define PSRAM_RSW_CODE_SIZE		24k // 0x5000

#define PSRAM_RSW_DATA_ADDR		(PSRAM_RSW_CODE_ADDR + PSRAM_RSW_CODE_SIZE)
#define PSRAM_RSW_DATA_SIZE		2k // 0x800

// 0xA0000000--0xA01FFFFF
//#define ASW_ROM_ADDR                    0x80000000
//#define ASW_ROM_ADDR_END                0x801FFFFF
//#define ASW_ROM_SIZE                    ((ASW_ROM_ADDR_END + 1) - ASW_ROM_ADDR) // 2Mbytes

// 0xA0020000--0xA0047FFF
#define HSM_ROM_ADDR                    0x80020000
#define HSM_ROM_ADDR_END                0x80047FFF
#define HSM_ROM_SIZE                    ((HSM_ROM_ADDR_END + 1) - HSM_ROM_ADDR) // 160K
#define HSM_ROM_OFFSET                  0x20000
#define HSM_ROM_ADDR2                   0x80320000 // PFLS1 HSM
#endif /* <= @SURE - RSW Code */
...
#    endif
  ...
  section_layout :vtc:linear
  {
    ...
    {
        #if 1
        group FLSLOADER_CODE (ordered, attributes=rwx, copy, run_addr=PSRAM_RSW_CODE_ADDR)
        {
            select ".text.Rsw.Rsw_Main"; /* Should assign the Rsw_Main function first */
            select ".text.Rsw*.*";
        }
	#endif
        #if 0
        group FLSLOADER_CODE (ordered, attributes=rwx, copy, run_addr=PSRAM_RSW_CODE_ADDR)
        {
            select ".text.Rsw.Rsw_Main"; /* Should assign the Rsw_Main function first */
            select ".text.Rsw*.*";
            select ".text.TestGpt.IoHwAb_GptNotification0";
            select ".text.TestApp._Task10us";
            select ".text.TestApp._Task1ms";
            select ".text.TestApp._Task5ms";
            select ".text.Cmic_Manager.CmicM_SetSysTick";
        }
	#endif
        #if 1
        group (ordered, attributes=rw, run_addr=PSRAM_RSW_DATA_ADDR)
        {
            select ".bss.Rsw_Data.*";
            select ".data.Rsw_Data.*";
        }
        #endif
        #if 1 /* PFLS0 HSM */
        group
        {
            /* PFLS0 HSM */
            group hsm_code1 (ordered, align = 4, run_addr=HSM_ROM_ADDR)
            {
	         section "hsm_code1" (size=HSM_ROM_SIZE, attributes=rx, fill=0)
	         {
	         }
            }
        }
        ...
        {
            /* PFLS1 HSM */
            group hsm_code2 (ordered, align = 4, run_addr=HSM_ROM_ADDR2)
            {
	         section "hsm_code2" (size=HSM_ROM_SIZE, attributes=rx, fill=0)
	         {
	         }
            }
        }
