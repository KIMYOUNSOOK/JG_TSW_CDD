...
static void Fls_lEraseCmdCycles(const uint32 EraseAddress)
{
  ...
  /*MISRA2012_RULE_11_4_JUSTIFICATION: Conversion between pointer to
  object and integer type is permitted for special function registers*/
  /*MISRA2012_RULE_13_5_JUSTIFICATION: Special function registers are
   used for comparison with 'Innercount'(non-volatile entity). There
   are no side affects of this comparison.*/
                                ...
                                const Fls_17_Dmu_AddressType* DataPtr)
{
  ...
  PageDataPtr = NULL_PTR;
  PageDataPtrCpy = NULL_PTR;
  ...
  */
  ...
  if (((uint32)DataPtr & FLS_WORD_SIZE_MASK) != 0)
  {
    ...
    contents of the pointer. The data is copied bytewise and assigned to start
    address */
  ...
  */
  ...
  if (((uint32)DataPtr & FLS_WORD_SIZE_MASK) != 0)
  {
    PageDataPtr[0] = PageDataPtrCpy[4];
    PageDataPtr[1] = PageDataPtrCpy[5];
    PageDataPtr[2] = PageDataPtrCpy[6];
    PageDataPtr[3] = PageDataPtrCpy[7];
  }
  else
  {
    PageData = DataPtr[1];
