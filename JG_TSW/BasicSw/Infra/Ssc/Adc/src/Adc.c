...
#if ( ADC_SW_MINOR_VERSION != 0U )
  #error "ADC_SW_MINOR_VERSION does not match."
#endif

#if ( ADC_SW_PATCH_VERSION != 1U )
...
#if (ADC_ENABLE_START_STOP_GROUP_API == STD_ON)
/*CYCLOMATIC_Adc_StartGroupConversion_JUSTIFICATION: This API starts the
conversion for the requested ADC Group for different modes of ADC operation
(No priority, Queuing, HW priority & HW-SW priority) and performs the DET or
Safety or Runtime Error check for AR422 and AR440 based on the enable of
pre-compile macros. The different if conditions in different operating modes
cause the complexity to increase, but actually during run-time out of all if
conditions many will be pre-compile time removed*/
/*******************************************************************************
...
#if (ADC_ENABLE_START_STOP_GROUP_API == STD_ON)
/*CYCLOMATIC_Adc_StopGroupConversion_JUSTIFICATION: This API stops the ongoing
conversion for the requested ADC Group for different modes of ADC operation
(No priority, Queuing, HW priority & HW-SW priority) and performs the DET or
Safety or Runtime Error check for AR422 and AR440 based on the enable of
pre-compile macros. The different if conditions in different operating modes
...
#if (ADC_HW_TRIGGER_API == STD_ON)
/*CYCLOMATIC_Adc_DisableHardwareTrigger_JUSTIFICATION: This API stops the
ongoing conversion and disables the hardware trigger for the requested ADC
Group for different modes of ADC operation (No priority, Queuing, HW priority
& HW-SW priority) and performs the DET or Safety or Runtime Error check for
AR422 and AR440 based on the enable of pre-compile macros. The different if
conditions in different operating modes cause the complexity to increase, but
