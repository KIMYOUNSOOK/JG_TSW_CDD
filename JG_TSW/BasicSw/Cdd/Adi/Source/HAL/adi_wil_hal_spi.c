...
typedef struct {
    uint8                   m_nSpiState; //0:Disable  1:Enable
    boolean                 m_bSpiTransmitState;  //0:Free 1:Busy
    adi_wil_hal_spi_cb_t    m_cbPtr;                      /* SPI callback function ptr  */
} spi_port_state_t;
...
void CMIC1_SeqEndNotification(void)
{
     if (spiPort[0].m_cbPtr != (void *)0)
    {
        spiPort[0].m_cbPtr(PORT0_SPI_DEVICE, PORT0_CHIP_SELECT);
    }
      spiPort[0].m_bSpiTransmitState = 0; // free
}

/* Notification function for Sequence: SpiSequence_0_CMIC2 */
void CMIC2_SeqEndNotification(void)
{
    if (spiPort[1].m_cbPtr != (void *)0)
    {
        spiPort[1].m_cbPtr(PORT1_SPI_DEVICE, PORT1_CHIP_SELECT);
    }
      spiPort[1].m_bSpiTransmitState = 0; // free
}
...
adi_wil_hal_err_t adi_wil_hal_SpiInit(uint8 iSPIDevice, adi_wil_hal_spi_cb_t pfCb)
{
    ...
    adi_wil_hal_err_t rc = ADI_WIL_HAL_ERR_SUCCESS;
                                          ...
                                          uint16 iLength)
{
    ...
    Spi_SeqResultType result = SPI_SEQ_OK;
    Std_ReturnType stdResult = E_OK;
    UInt32 nSpiTryCnt = 0U;
    Spi_ChannelType Channel = CMIC_SPI_M_PORT_CHANNEL;
    Spi_SequenceType Sequence = CMIC_SPI_M_PORT_SEQUENCE;
    (void)iChipSelect; /* chip select not used as there is only one device on each port */

// *** 함수를 2개로 나눠서 호출하도록 변경할것  $$$
    if (spiPort[iSPIDevice].m_bSpiTransmitState)
    {
        if(spiPort[iSPIDevice].m_cbPtr){
             spiPort[iSPIDevice].m_cbPtr(iSPIDevice, 0);
        }
        return ADI_WIL_HAL_ERR_SUCCESS;
    }

    spiPort[iSPIDevice].m_bSpiTransmitState = TRUE; //Busy

    if (iSPIDevice == PORT1_SPI_DEVICE){
        Channel = CMIC_SPI_M2_PORT_CHANNEL;
        Sequence = CMIC_SPI_M2_PORT_SEQUENCE;
    }
    stdResult = Spi_SetupEB(Channel, pTx, pRx, iLength);
    stdResult |= Spi_AsyncTransmit(Sequence);

    if(stdResult != E_OK)
    {
        result = SPI_SEQ_FAILED;
    }

    nSpiTryCnt = 0;

    spiPort[iSPIDevice].m_bSpiTransmitState = 0; // free
