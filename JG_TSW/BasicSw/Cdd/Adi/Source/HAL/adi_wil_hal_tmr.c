...
adi_wil_hal_err_t adi_wil_hal_TmrStart(adi_wb_hal_tmr_cb_t pfCb)
{
    ...
    {
#if 1 // Using MCAL GPT Interrupt TIMER
        Gpt_StartTimer(GptConf_GptChannelConfiguration_GptChannelConfiguration_1, HAL_TMR_PERIOD_MicroSEC*100U); // 3ms
#else
        _TmrId = TestGpt_RegTimer(HAL_TMR_PERIOD_MicroSEC, GPT_CONTINUE, _HalTmrIsr);
